...
При открытии приложения на экран выводится окно, на котором пользователь может увидеть две вкладки: "Анализ цен" и "Сравнение цен", причём та вкладка, которая не закрашена, показывает в каком режиме находится пользователь.

Если открыта вкладка "Анализ цен", то пользователь должен выполнять следующие действия:
...
- как только все необходимые данные корректно введены, то если пользователь нажмёт кнопку "Построить график", то ему соответственно выведется в отдельном окне график соотношения закрывающих цен акций компаний выбранного типа в выбранной стране с днями в выбранном периоде, а если будет нажата кнопка "Информация о компаниях", то в отдельном окне появится краткий профиль для каждой компании выбранного типа в выбранной стране.

Если открыта вкладка "Сравнение цен", то пользователь должен выполнять следующие действия:
